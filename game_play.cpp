...
/* 게임보드와 블록을 오버랩해서 overlap_board에 반환한다
   게임보드와 블록이 아무것도 존재하지 않는 경우 0
   게임보드 또는 블록 둘중 하나만 존재하는 경우 1
   게임보드와 블록이 중복되는 경우 2
   로 설정한다 */
void overlap_board_and_block(int (*game_board)[9], int (*block)[9],
                             int (*overlap_board)[9]) {
    overlap_board[9][9] = {
        0,
    }; // overlap_board를 0으로 초기화

    for (int i = 0; i < 9; i++) {
        for (int j = 0; j < 9; j++) {
            // overlap_board 업데이트
            overlap_board[i][j] = game_board[i][j] + block[i][j];
        }
    }
}
